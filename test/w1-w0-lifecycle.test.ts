...
describe('w1 w0/lifecycle (88-118, 123-127)', () => {
  ...
  test('126+127: worktree deltas stay inside the frozen allowlist', async () => {
    ...
    // Cross-guard recognition (Desktop snapshot-contract/coherence remediation gate):
    // the three guard files' own additive-ratification edits ride in `edits`;
    // all seven historical entries unchanged and in order.
    const edits = new Set(['src/desktop/protocol.ts', 'src/desktop/control-service.ts', 'src/desktop/control-core.ts', 'src/cli.ts', 'src/desktop/shell/ControlClient.cs', 'src/desktop/shell/Program.cs', 'src/desktop/shell/TrayIcon.cs', 'test/w0-regression.test.ts', 'test/w1-w0-lifecycle.test.ts', 'test/w2-frozen-boundary.test.ts'])
    // Additive ratification (Desktop snapshot-contract/coherence remediation):
    // new remediation evidence files; all historical entries unchanged.
    const news = new Set(['src/desktop/web-bridge.ts', 'src/desktop/web-assets.ts', 'src/desktop/web-launch.ts', 'test/w1-listener-origin.test.ts', 'test/w1-bootstrap.test.ts', 'test/w1-session-csrf.test.ts', 'test/w1-http-dto-allowlist.test.ts', 'test/w1-w0-lifecycle.test.ts', 'docs/web-control.md', 'src/desktop/shell/SnapshotContractSelftest.cs', 'test/desktop-snapshot-coherence.test.ts'])
